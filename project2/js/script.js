...
$.ajax({
  ...
  success: function (data) {
    data.forEach(function (e) {
      $("#personnel").append(
        `<div class=" col-md-4 mb-2 person" data-first=${e.firstName} data-last=${e.lastName} data-id=${e.id}  data-email= ${e.email}>
         ...
         <p class="mb-1"> ${e.email} </p></div> <div><div class="d-flex w-100 justify-content-between"><button type="button" class="col-4 btn btn-sm ml-3 mr-1" onclick="editEmployee(${e.id})" name="id"> Edit </button><button type="submit" class="col-4 btn btn-sm ml-5" name="id" value="
    ...
    data.forEach(function (e) {
      $("#departmentModal").append(
        `<tr><td> ${e.name} </td><td><button type="submit" class="btn btn-sm"name="id" data-id="${e.id}" value="${e.id}"
         onclick="deleteDepartment(${e.id})">Delete</button></td>
         </tr>`
      );
    });
